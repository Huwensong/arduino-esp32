...
NetworkEvents::NetworkEvents() : _arduino_event_group(NULL), _arduino_event_queue(NULL),
...
bool NetworkEvents::initNetworkEvents() {
    ...
    if (!_arduino_event_task_handle) {
        xTaskCreateUniversal(_network_event_task, "arduino_events", 4096, this, ESP_TASKD_EVENT_PRIO - 1,
                             &_arduino_event_task_handle, ARDUINO_EVENT_RUNNING_CORE);
        if (!_arduino_event_task_handle) {
...
void NetworkEvents::checkForEvent() {
    ...
    for (uint32_t i = 0; i < cbEventList.size(); i++) {
        ...
        log_d("[[[[[[[[[[[5test ");
        vTaskDelay(500);
        log_d("[[[[[[[[[[[5test2 | %u",cbEventList.size());
...
void NetworkEvents::removeEvent(NetworkEventFuncCb cbEvent, arduino_event_id_t event) {
    log_d("rrrrrrrrrrrrrrrrr remove");
    if (!cbEvent) {
        return;
    }

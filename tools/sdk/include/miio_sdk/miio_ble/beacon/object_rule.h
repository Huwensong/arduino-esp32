...
typedef struct {
    uint16_t pid;//for check rule by pid
    uint16_t obj_id;
    uint32_t interval;
    uint32_t delta;
} object_rule_t;
...
int object_rule_delete(uint16_t pid,uint16_t obj_id);
int object_rule_delete_by_pid(uint16_t pid);
object_rule_t *object_rule_search(uint16_t pid,uint16_t obj_id);

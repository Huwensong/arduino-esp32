...
typedef enum {
    ...
    MIBLE_RPC_SPEC_QUERY_PROB,
    MIBLE_RPC_PROPERTIES_CHANGED,
    MIBLE_RPC_EVENT_OCCURED,
    MIBLE_RPC_REPORT_EVT,
    MIBLE_RPC_KEEP_ALIVE,
    MIBLE_RPC_GATEWAY_ENABLE,
    MIBLE_RPC_QUERY_DEV_LIST,
...
int mible_rpc_query_dev_list(void);
void test_qurey_list_cb(void);

int mible_rpc_spec_query_prod(uint16_t pid, uint8_t offset);
int mible_rpc_properties_changed(authed_dev_t *p_dev, mibeacon_object_array_t *p_object_array);
int mible_rpc_event_occured(authed_dev_t *p_dev, mibeacon_object_array_t *p_object_array);
...
int mible_rpc_statistic(const char *title, const char *content);
